...
def perform_initial_permutation(text):
    binary_form = bin(int(text,16))[2:]
    len_of_binary = len(binary_form)
    for i in range(len_of_binary,64):
        binary_form = '0' + binary_form
    new_text = ""
    for i in initial_permutation:
        new_text += binary_form[i-1]
    return new_text
...
plain_text = input()
plain_text = convert_plaintext_to_hexadecimal(plain_text)
len_of_plaintext = len(plain_text)

subkey_generation()
ciphertext = ""
for j in range(0,len_of_plaintext,16):
    if (j + 16) > len_of_plaintext:
        plain = plain_text[j:]
        lenOfPlain = len(plain)
        for k in range(lenOfPlain,16):
            plain += '0'
    else:
        plain = plain_text[j:j+16]
    after_IP = perform_initial_permutation(plain)
    Lo = after_IP[:32]
    Ro = after_IP[32:]
    for k in range(16):
        perform_f_function = f_function(Ro,subKey[k])
        temp = Ro
        Ro = XOR(perform_f_function,Lo)
        Lo = temp
    round_ciphertext = apply_final_permutation(Ro+Lo)
    ciphertext += convert_binary_to_hexadecimal(round_ciphertext)
